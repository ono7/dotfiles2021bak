...
def pattern_gen(length):
    ...
    if length >= MAX_PATTERN_LENGTH:
        raise MaxLengthException(
            "ERROR: Pattern length exceeds maximum of %d" % MAX_PATTERN_LENGTH
        )
...
def pattern_search(search_pattern):
    ...
    raise WasNotFoundException(
        "Couldn`t find %s (%s) anywhere in the pattern." % (search_pattern, needle)
    )
...
if __name__ == "__main__":
    ...
    if sys.argv[1].isdigit():
        ...
        try:
            ...
            print(
                "Pattern %s first occurrence at position %d in pattern."
                % (sys.argv[1], found)
            )
